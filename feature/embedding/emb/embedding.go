...
func (e Embedding) Validate() error {
	if e.Word == "" {
		...
		return fmt.Errorf("Dim of %s is zero", e.Word)
	} else if e.Dim != len(e.Vector) {
		return fmt.Errorf("dim and length of Vector must be same, Dim=%d, len(Vec)=%d", e.Dim, len(e.Vector))
	}
	return nil
}
...
func (embs Embeddings) Validate() error {
	if len(embs) > 0 {
		...
		for _, emb := range embs {
			if dim != emb.Dim {
				return fmt.Errorf("dimension for all vectors must be the same: %d but got %d", dim, emb.Dim)
			}
		}
	}
	return nil
